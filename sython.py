...
async def join_channel(event):
    try:
        await sython(JoinChannelRequest("@GHHGBFH"))
    except BaseException:
        pass
...
async def join_channel(event):
    try:
        await sython(JoinChannelRequest("@Dython"))
    except BaseException:
        pass  
...
async def _(event):
    ...
    await sython.send_file(
        "me", pic, caption=f"تـم حفظ الصورة او الفيديو الذاتي هنا : DARK"
    )
...
async def _(event):
    ...
    await event.edit(f'''
♔ DARK SOURCE IS WORKING
╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌
❖╎WELCOME TO DARK SOURCE
❖╎𝐝𝐞𝐯𝐥𝐨 : DARK 
❖╎𝐜𝐡𝐚𝐧𝐧𝐞𝐥 : @@A_S_D_F_S
❖╎𝐯𝐞𝐫𝐬𝐢𝐨𝐧 : 2.7 - revised 
❖╎user : @EEYEEW
